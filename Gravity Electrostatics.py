...
class FCurve:
    def __init__(self, q_1, q_2, C, a, k=1):
        self.k = k
        self.q_1 = q_1
        self.q_2 = q_2 
        self.C = C
...
def makeVectors(moment):
    ...
    vecs = np.zeros((5,3),dtype=Vector)
    players = []
...
#    system = np.concatenate((system,zeros),axis = 1)
    def makefunc(dists,trigMat):
        def func(weights):
            weights = np.reshape(weights,(4,5))
            ...
            for d in defenseData:
                curves = []
                for o in range(offenseData.shape[1]):
                    curves.append(FCurve(d,
                                         offenseData[0][o],
                                         offenseData[1][o],
                                         offenseData[2][o]))
                curves.append(FCurve(d,NET_WEIGHT,NET_C,NET_A))
                curves.append(FCurve(d,BALL_WEIGHT,BALL_C,BALL_A))
    ...
    initialMat = np.ones((4,5))
    #initialMat[0] = initialMat[0]*-1
    #print(initialMat)
    ans = scipy.optimize.minimize(myFunc,initialMat)
#    print(ans)
    solution = ans.x
    solution = np.reshape(solution,(4,5))
#    print(solution)
    radii = []
    for count,off in enumerate(solution[0]):
        rSum = 0
        els = 0
        for Def in solution[1]:
            thisCurve = FCurve(off,Def,solution[2][count],solution[3][count])
            try:
                root = scipy.optimize.root_scalar(thisCurve.curveFunc,bracket = [0.01,3]).root
            except:
                continue
#            if(root > 10):
#                continue
